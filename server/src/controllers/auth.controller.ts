...
export const forgotPassword = async (
  ...
  try {
    ...
    if (!user) {
      // Для безпеки не повідомляємо, що користувача не існує
      // Просто повертаємо успішну відповідь (захист від перевірки номерів)
      res.status(200).json({
        success: true,
        message: 'Якщо користувач з таким номером існує, код верифікації відправлено',
      });
      return;
    }
